...
include("BarbarianEraScripts")
include("BarbarianGGScripts")
...
function OnPopup( popupInfo )
	...
	eraID = load( aPlayer, "Last Era Changed")
	if eraID == nil then eraID = 0 end

	if eraID == 0 or eraID == 1 then
		adj = "Barbarian";
		camp = "Barbarian Encampment";
	end

	if isgeneral == 1 then
		local iNumGold = iNumGold + goldbonus
		Controls.DescriptionLabel:SetText(Locale.ConvertTextKey("TXT_KEY_BARB_CAMP_GENERAL_CLEARED", adj, camp, iNumGold, culturebonus));
	else
		Controls.DescriptionLabel:SetText(Locale.ConvertTextKey("TXT_KEY_BARB_CAMP_CLEARED", camp, iNumGold));
	end
