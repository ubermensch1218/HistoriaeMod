...
function GoldTipHandler( control )
	...
	-- HM modified. 1
	local fTourGold = pPlayer:GetTourism();
	local fTotalIncome = fGoldPerTurnFromCities + iGoldPerTurnFromOtherPlayers + fCityConnectionGold + iGoldPerTurnFromReligion + fTradeRouteGold + fTraitGold + fTourGold;
	...
	if (math.floor(fTourGold) > 0) then
		strText = strText .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_TOURISM", math.floor(fTourGold));
	end
...
function GoldenAgeTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		if (pPlayer:GetGoldenAgeTurns() > 0) then
			...
			local iHappiness = pPlayer:GetExcessHappiness();
			local i2Happiness = iHappiness * 2
			...
			if (iHappiness >= 0) then
				if pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_BABYLON"] then
					strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_ADDITION", i2Happiness);
				else
					strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_ADDITION", iHappiness);
				end
			else
				if pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_BABYLON"] then
					strText = strText .. "[COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_LOSS", -i2Happiness) .. "[ENDCOLOR]";
				else
		...
		if (pPlayer:GetGoldenAgeTurns() > 0 and pPlayer:GetGoldenAgeTourismModifier() > 0) then
			strText = strText .. "[NEWLINE][NEWLINE]";
			if pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_BRAZIL:"] then
